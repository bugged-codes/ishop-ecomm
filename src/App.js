...
const App = () => {
	...
	useEffect(() => {
		...
		axios.get(`${API}/mobiles`).then(({ data }) => setMobiles(data));
	}, []);
...
function PhoneList() {
	...
	return <div className="phone-list-container">{mobiles && mobiles.map((mobile) => <Phone mobile={mobile} key={mobile._id} />)}</div>;
}
...
function Phone({ mobile }) {
	...
	return (
		<div className="phone-container">
			...
			<button className="add-btn" onClick={() => updateCart({ mobile, type: 'increment' })}>
		...
		<div className="cart-container">
			...
			<div className="description-product">
				...
				<div className="cart-item-button">
					<button className="incre-decre-btn" onClick={() => updateCart({ mobile, type: 'increment' })}>
						+
					</button>
					<span> Quantity : {mobile.qty} </span>
					<button className="incre-decre-btn" onClick={() => updateCart({ mobile, type: 'decrement' })}>
						-
					</button>
				</div>
			</div>
